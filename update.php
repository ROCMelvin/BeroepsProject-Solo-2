...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      ...
      if($verify_name->rowCount() > 0){
         ...
         $success_msg[] = 'Gebruikersnaam bijgewerkt!';
      }
   }
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         $warning_msg[] = 'Oud wachtwoord komt niet overeen!';
      }elseif($c_pass != $new_pass){
         $warning_msg[] = 'Nieuw wachtwoord komt niet overeen!';
      }else{
         if($new_pass != $empty_pass){
            $update_password = $conn->prepare("UPDATE `admins` SET password = ? WHERE id = ?");
            $update_password->execute([$c_pass, $admin_id]);
            $success_msg[] = 'wachtwoord bijgewerkt!';
         }else{
            $warning_msg[] = 'Voer een nieuw wachtwoord in!';
         }
      }
   }
